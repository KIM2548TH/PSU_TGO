...
def generate_bar_chart_base64(current_data, previous_data=None):
    ...
    previous_values = [previous_data.get(m, 0) for m in month_names] if previous_data else None
    fig, ax = plt.subplots(figsize=(6, 3))
    y_min, y_max = 0, max(current_values + (previous_values if previous_values else [])) if (current_values or previous_values) else 1
    ...
    ax.bar([i - bar_width/2 for i in x], previous_values if previous_values else [0]*12, width=bar_width, color="#4F46E5", label=f"{int(selected_year)-1}", zorder=2)
    ax.bar([i + bar_width/2 for i in x], current_values, width=bar_width, color="#10B981", label=f"{int(selected_year)}", zorder=2)
    ...
    try:
        ...
        font_prop = None
    legend = ax.legend(title="ปี ค.ศ.", prop=font_prop, title_fontproperties=font_prop, loc="upper right", bbox_to_anchor=(1.12, 1))
    plt.tight_layout()
    buf = io.BytesIO()
...
def download_pdf():
    ...
    try:
        ...
        data = calculate_emissions_data(user, selected_sub_scopes, time_period, int(selected_year))
        last_year_data = calculate_emissions_data(user, selected_sub_scopes, time_period, int(selected_year) - 1)
        bar_chart_base64 = generate_bar_chart_base64(data.get("daily_data", {}), last_year_data.get("daily_data", {}))
        category_chart_base64 = generate_category_chart_base64(data.get("category_data", {}))
        scope_emissions = {label: round(data["category_data"].get(label, 0), 2) for label in ["Scope 1", "Scope 2", "Scope 3"]}
        total = sum([data["category_data"].get(label, 0) for label in ["Scope 1", "Scope 2", "Scope 3"]])
        scope_breakdown_table = []
        ...
        uni_style = ParagraphStyle(
            'UniName', parent=styles['Normal'], fontSize=13, alignment=1, fontName=thai_font_name, textColor=colors.HexColor('#444'))
        ...
        meta_data = [
            ...
            [f"สร้างรายงานเมื่อ: {generated_date}", f"Total Scopes: {len(selected_scopes)}"]
        ]
        ...
        custom_label_spacing = 1  # spacing between label lines (e.g. label and sublabel) - no extra gap
        ...
        card1 = Table([
            [f"{data['total_emissions']:,.2f} tonCO₂e"],
            ["Total CO₂ Emissions"]
        ], colWidths=[card_width], rowHeights=[row_height, row_height])
        ...
        avg_label = "Per Week" if data['average_label'] == "Per Week" else data['average_label']
        card2 = Table([
        ...
        cards_row = Table([
            [card1, '', card2, '', card3]
        ], colWidths=[card_width, card_gap, card_width, card_gap, card_width], rowHeights=[card_height])
        ...
        if data.get('scope_data'):
            scope1_subscopes = [info for info in data['scope_data'].values() if info['ghg_scope'] == 1]
            scope1_subscopes = sorted(scope1_subscopes, key=lambda x: x['emissions'], reverse=True)[:3]
            top_subscope_header = ['#', 'Subscope Name', 'Scope/Subscope', 'Emissions (tCO₂e)']
            top_subscope_rows = []
            for idx, subscope in enumerate(scope1_subscopes, 1):
                scope_sub_label = f"{subscope['ghg_scope']}.{subscope['ghg_sup_scope']}"
                subscope_name_para = Paragraph(subscope.get('ghg_name', ''), ParagraphStyle('SubscopeName', fontName=thai_font_name, fontSize=9, wordWrap='CJK'))
            ...
            col_widths = [0.5/5.1*total_table_width, 2.2/5.1*total_table_width, 1.2/5.1*total_table_width, 1.2/5.1*total_table_width]
            top_subscope_table = Table(top_subscope_table_data, colWidths=col_widths)
            top_subscope_table.setStyle(TableStyle([
                ('FONTNAME', (0, 0), (-1, -1), thai_font_name),
                ('FONTSIZE', (0, 0), (-1, -1), 9),
            ...
            story.append(Paragraph("Top Subscopes for Scope 1", ParagraphStyle('TopSubscopeTitle', fontName=thai_font_bold, fontSize=13, textColor=colors.HexColor('#155DFC'), spaceAfter=9, alignment=0)))
            story.append(top_subscope_table)
            story.append(Spacer(1, 16))
        # Top Subscopes for Scope 2
        if data.get('scope_data'):
            scope2_subscopes = [info for info in data['scope_data'].values() if info['ghg_scope'] == 2]
            scope2_subscopes = sorted(scope2_subscopes, key=lambda x: x['emissions'], reverse=True)[:3]
            top_subscope_header2 = ['#', 'Subscope Name', 'Scope/Subscope', 'Emissions (tCO₂e)']
            top_subscope_rows2 = []
            for idx, subscope in enumerate(scope2_subscopes, 1):
                scope_sub_label = f"{subscope['ghg_scope']}.{subscope['ghg_sup_scope']}"
                subscope_name_para2 = Paragraph(subscope.get('ghg_name', ''), ParagraphStyle('SubscopeName2', fontName=thai_font_name, fontSize=9, wordWrap='CJK'))
            ...
            top_subscope_table2.setStyle(TableStyle([
                ('FONTNAME', (0, 0), (-1, -1), thai_font_name),
                ('FONTSIZE', (0, 0), (-1, -1), 9),
                # Header: background = text color, text = white
            ...
            story.append(Paragraph("Top Subscopes for Scope 2", ParagraphStyle('TopSubscopeTitle2', fontName=thai_font_bold, fontSize=13, textColor=colors.HexColor('#36A63E'), spaceAfter=9, alignment=0)))
            story.append(top_subscope_table2)
            story.append(Spacer(1, 16))

        # Top Subscopes for Scope 3
        if data.get('scope_data'):
            scope3_subscopes = [info for info in data['scope_data'].values() if info['ghg_scope'] == 3]
            scope3_subscopes = sorted(scope3_subscopes, key=lambda x: x['emissions'], reverse=True)[:3]
            top_subscope_header3 = ['#', 'Subscope Name', 'Scope/Subscope', 'Emissions (tCO₂e)']
            top_subscope_rows3 = []
            for idx, subscope in enumerate(scope3_subscopes, 1):
                scope_sub_label = f"{subscope['ghg_scope']}.{subscope['ghg_sup_scope']}"
                subscope_name_para3 = Paragraph(subscope.get('ghg_name', ''), ParagraphStyle('SubscopeName3', fontName=thai_font_name, fontSize=9, wordWrap='CJK'))
            ...
            top_subscope_table3.setStyle(TableStyle([
                ('FONTNAME', (0, 0), (-1, -1), thai_font_name),
                ('FONTSIZE', (0, 0), (-1, -1), 9),
                # Header: background = text color, text = white
            ...
            story.append(Paragraph("Top Subscopes for Scope 3", ParagraphStyle('TopSubscopeTitle3', fontName=thai_font_bold, fontSize=13, textColor=colors.HexColor('#9810FA'), spaceAfter=9, alignment=0)))
        ...
        # Table header
        compare_header = ["เดือน", f"{int(selected_year)-1} tCO₂e", f"{selected_year} tCO₂e"]
        compare_rows = []
        for i, m_th in enumerate(month_names_th):
            compare_rows.append([
                m_th,
                f"{last_monthly[i]:,.2f}",
                f"{current_monthly[i]:,.2f}"
            ])
        compare_table_data = [compare_header] + compare_rows
        # Table width matches cards row
        total_table_width = 3*card_width + 2*card_gap
        col_widths = [1.5/5.1*total_table_width, 1.8/5.1*total_table_width, 1.8/5.1*total_table_width]
        ...
        story.append(Paragraph("ตารางเปรียบเทียบค่า tCO₂e รายเดือน ({} vs {})".format(int(selected_year)-1, selected_year), ParagraphStyle('CompareTitle', fontName=thai_font_bold, fontSize=13, textColor=colors.HexColor('#F54900'), spaceAfter=9, alignment=0)))
        ...
        total_emissions = sum([data["category_data"].get(label, 0) for label in ["Scope 1", "Scope 2", "Scope 3"]])
        ...
        for label, color in zip(["Scope 1", "Scope 2", "Scope 3"], ["#4f46e5", "#10b981", "#f97316"]):
            ...
            breakdown_table_rows.append([
                Paragraph(label, ParagraphStyle('ScopeLabel', fontName=thai_font_bold, fontSize=10, textColor=colors.HexColor(color))),
                f"{percent:.1f}%",
                f"{value:,.2f}"
            ])
        breakdown_table_data = [breakdown_table_header] + breakdown_table_rows
        table_width = 3.5*inch
        col_widths = [1.2/4.4*table_width, 1.0/4.4*table_width, 2.2/4.4*table_width]
        breakdown_table = Table(breakdown_table_data, colWidths=col_widths, rowHeights=[22] + [28]*len(breakdown_table_rows))
        ...
        right_table = RLTable([[Spacer(1, 40)], [breakdown_table], [Spacer(1, 40)]], colWidths=[3.5*inch])
        right_table.setStyle(RLTableStyle([
            ('ALIGN', (0, 1), (0, 1), 'RIGHT'),
        ...
        row_table = RLTable([[chart_img, right_table]], colWidths=[2.7*inch, 3.5*inch])
        row_table.setStyle(RLTableStyle([
            ('VALIGN', (0, 0), (0, 0), 'TOP'),
        ...
        if notes.strip():
            ...
            story.append(Spacer(1, 20))
            notes_header = Paragraph("หมายเหตุ", ParagraphStyle('NotesHeader', fontName=thai_font_bold, fontSize=13, textColor=colors.HexColor('#155DFC'), spaceAfter=9, alignment=0))
            story.append(notes_header)
            notes_para = Paragraph(decoded_notes, ParagraphStyle('NotesText', fontName=thai_font_name, fontSize=11, textColor=colors.HexColor('#444'), leading=16))
